...
void *poll_init(struct event_loop *eventLoop) {
  ...
  // 用-1表示这个数组位置还没有被占用
  int i;
  for (i = 0; i < INIT_POLL_SIZE; i++) pollDispatcherData->event_set[i].fd = -1;
...
int poll_add(struct event_loop *eventLoop, struct channel *channel1) {
  ...
  msgx("poll added channel fd==%d, %s", fd, eventLoop->thread_name);
  if (i >= INIT_POLL_SIZE) {
...
int poll_del(struct event_loop *eventLoop, struct channel *channel1) {
  ...
  msgx("poll delete channel fd==%d, %s", fd, eventLoop->thread_name);
  if (i >= INIT_POLL_SIZE) {
...
int poll_update(struct event_loop *eventLoop, struct channel *channel1) {
  ...
  msgx("poll updated channel fd==%d, %s", fd, eventLoop->thread_name);
  if (i >= INIT_POLL_SIZE) {
...
int poll_dispatch(struct event_loop *eventLoop, struct timeval *timeval) {
  ...
  for (i = 0; i < INIT_POLL_SIZE; i++) {
    ...
    if ((socket_fd = pollfd.fd) < 0) continue;

    //有事件发生
    if (pollfd.revents > 0) {
      msgx("get message channel i==%d, fd==%d, %s", i, socket_fd,
